package nl.bs10.gmaps.components
{
	...
	public class BrightMarker extends Marker
	{
		...
		public function BrightMarker(arg0:LatLng, arg1:MarkerOptions=null) {
		...
		public function set parent(value:AppController):void {
			if(value !== _parent) {
				if(_parent)
		...
		public function get parent():AppController {
			return _parent;
		}
		...
		private function _markerClickHandler(event:MapMouseEvent):void {
			parent.markerClickHandler(markerprops);
		...
		private function _hoverHandler(event:MapMouseEvent):void {
			if(event.type == MapMouseEvent.ROLL_OVER) {
				...
				parent.showMarkerDelete(foreground.getBounds(parent), markerprops.markerId);
			} else {
				clearTimeout(parent.deleteMarkerTimeout);
