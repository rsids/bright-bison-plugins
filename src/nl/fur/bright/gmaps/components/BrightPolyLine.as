package nl.fur.bright.gmaps.components
{
	...
	public class BrightPolyLine extends OverlayBase implements IBrightPolyLine, IOverlay
	{
		...
		public function get color():uint {
			return _polyPage.color;
		}
		
		public function set polyPage($value:PolyPage):void	{
			_polyPage = $value;
		}
		...
		public function getPosition(lat:Number, lng:Number):Point {
			return pane.map.fromLatLngToViewport(new LatLng(lat,lng));
		}
		...
		private function onPointMouseOver(event:BrightPointEvent):void
		{
			if (_isEnded)
			{
				dispatchEvent(new BrightPolyLineEvent(BrightPolyLineEvent.POLYLINE_POINT_MOUSE_OVER, _polyPage, event.target as IBrightPoint));
			}
		}
		...
		private function onPointMouseOut(event:BrightPointEvent):void
		{
			if (_isEnded)
			{
				dispatchEvent(new BrightPolyLineEvent(BrightPolyLineEvent.POLYLINE_POINT_MOUSE_OUT, _polyPage, event.target as IBrightPoint));
			}
		}
